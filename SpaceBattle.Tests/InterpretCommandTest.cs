...
{
    private readonly Dictionary<string, Queue<IComand>> games = new Dictionary<string, Queue<IComand>>();
    private readonly Dictionary<string, UniversalyObject> gameUObjectMap = new Dictionary<string, UniversalyObject>();
    ...
    public TestInterpretCommand()
    {
        ...
        {
            if (IoC.Resolve<IDictionary<string, Queue<IComand>>>("QueueMap").TryGetValue((string)args[0], out var queue))
            {
                return queue;
            }
        ...
        {
            if (IoC.Resolve<IDictionary<string, UniversalyObject>>("UObjectMap").TryGetValue((string)args[0], out var uObject))
            {
                return uObject;
            }
    ...
    public void PushIsSuccess()
    {
        ...
        var mockMessage = new Mock<IOrderMessage>();
